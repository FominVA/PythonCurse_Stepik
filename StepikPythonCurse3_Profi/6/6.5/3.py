...
sorted_staff = sorted(staff_broken, key=lambda item: (item[0], item[1]))
result = defaultdict(list)

for department, name in sorted_staff:
    result[department].append(name)

sorted_departments = sorted(result.keys())

for department in sorted_departments:
    employees = result[department]
    unique_sorted_employees = sorted(list(set(employees)))
    employees_str = ", ".join(unique_sorted_employees)
    
    # Выводим результат в требуемом формате
    print(f"{department}: {employees_str}")
